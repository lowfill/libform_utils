	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			addData: function (data) { //parse data
				if (p.dataType == 'json') {
					data = $.extend({rows: [], grid_page: 0, total: 0}, data);
				}
				if (p.preProcess) {
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row,table_) {
				...
				//TODO Added table container for avoid problems with IE
				table_ = document.createElement('table');
				$(table_).css("margin-top","0px");
				$(table_).append(tbody);
				$(t).append(table_);
